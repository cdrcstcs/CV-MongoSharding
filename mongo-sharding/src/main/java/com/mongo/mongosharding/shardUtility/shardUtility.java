...
public class shardUtility {
  ...
  public void setShardKey(String collectionName, String shardField) {
    ...
    final BasicDBObject cmd = new BasicDBObject("shardCollection", applicationProperties.getMongodb().getDatabase() + "." + collectionName);
    cmd.put("key", shardKey);
    try {
      result = mongoDatabase.runCommand(cmd);
      log.info("Added shard key: {} for collection {}, result: {}", shardField, collectionName, result);
    } catch (Exception e) {
  ...
  public MongoClient getMongoClient() {
    ...
    if (mongoClient != null)
      return mongoClient;
    try {
      MongoClientURI mongoClientURI = new MongoClientURI(applicationProperties.getMongodb().getUri());
  ...
  public void init() {
    ...
    List<ShardCollection> shardCollections = applicationProperties.getMongodb().getShardCollections();
